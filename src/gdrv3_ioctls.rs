...
impl GigabyteDriver {
    ...
    // Does not check if a page boundary is crossed TODO
    ...
    pub fn read_phys_mem(&self, physical_address: PhysicalAddres, size: usize) -> &[u8] {
        ...
        // Plus one to account for a buffer crossing page bounds
        let size_rounded = ((size + 4095) / 4096  + 1) * 4096;
        let phys_mem_section = self.ioctl_map_physmem(physical_address_page, size_rounded as u64);


        let phys_mem = unsafe { std::slice::from_raw_parts((phys_mem_section + physical_address_page_offset as u64) as *const u8, size) };
        let buffer =
            phys_mem.clone();
    ...
    pub fn write_phys_mem(&self, physical_address: PhysicalAddres, buffer: &[u8]) {
        let physical_address_page = physical_address & (0xffff_ffff_ffff_f000);
        let physical_address_page_offset = (physical_address & 0xfff) as usize;
        ...
        let phys_mem = unsafe { std::slice::from_raw_parts_mut((phys_mem_section + physical_address_page_offset as u64) as *mut u8, size) };
